...
export async function launchDeterministicFixtureMission(scenarioId: DeterministicScenarioId = "release_manifest_version_drift") {
  ...
  await appendMissionEvent({ missionId: mission.id, eventType: "VERIFICATION_PASSED", actor: "Verifier", tool: "fixture-isolation", result: scenario.verification.passedResult, evidenceRefs: [verificationEvidence.id, sandboxRun.id] });
  const approval = await addApprovalRequest({ missionId: mission.id, actionType: "SIMULATED_GITHUB_PULL_REQUEST", risk: repairProposal.risk, justification: scenario.verification.approvalJustification });
