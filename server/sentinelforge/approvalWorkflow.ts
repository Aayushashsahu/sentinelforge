...
export type PersistedMission = { id: string; status: MissionStatus; repository: string };
...
export type ApprovalWorkflowPort<TBundle = unknown> = {
  ...
  createSimulatedExternalAction(input: { missionId: string; target: string }): Promise<{ id: string; result: string }>;
...
export async function resolvePersistedApproval<TBundle>(port: ApprovalWorkflowPort<TBundle>, requestId: string, approve: boolean): Promise<TBundle> {
  ...
  const action = await port.createSimulatedExternalAction({ missionId: mission.id, target: mission.repository });
