...
export async function updateFixtureProofExternalAction(actionId: string, update: Pick<FixtureProofAction, "status" | "remote" | "failure">) {
  const db = await getDb(); if (!db) throw new Error("Database is unavailable.");
  const action = await getFixtureProofExternalAction(actionId);
  if (!action) throw new Error("Fixture proof action was not found.");
  const next = { ...action, status: update.status as FixtureProofActionStatus, remote: update.remote, ...(update.failure ? { failure: update.failure } : {}) };
