...
function responseError(operation: string, status: number): Error {
  return new Error(`Fixture GitHub ${operation} failed with HTTP ${status}.`);
}
...
export class GitHubFixtureWriteApi implements FixtureProofGitHubPort {
  ...
  private async request(operation: string, path: string, init: RequestInit = {}): Promise<unknown> {
    ...
    if (response.status < 200 || response.status >= 300) throw responseError(operation, response.status);
  ...
  async getBranchRef(branchName: string) {
    ...
    if (response.status !== 200) throw responseError("branch lookup", response.status);
