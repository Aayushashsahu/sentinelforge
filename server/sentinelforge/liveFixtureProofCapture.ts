...
import { buildStreamAuditInputs, readBoundedSessionHistory, readBoundedTurn } from "./liveWorkflow";
...
import { buildFixtureProviderNormalizedHistoryAuditInputs, buildFixtureProviderRawHistoryAuditInputs, normalizeFixtureProviderHistory } from "./fixtureProviderHistoryNormalization";
...
export async function runLiveFixtureProofApprovalCapture(input: { missionId: string; actionId: string }) {
  ...
  await readBoundedTurn({ client, sessionId: session.id, message: buildFixtureProofApprovalMessage({ missionId: input.missionId, actionId: action.id }) });
  const rawEvents = await readBoundedSessionHistory({ client, sessionId: session.id });
  await appendMissionEvents(buildFixtureProviderRawHistoryAuditInputs({ missionId: input.missionId, turnId: session.id, rawEvents }));
  const normalizedEvents = normalizeFixtureProviderHistory({ events: rawEvents, sessionId: session.id });
  await appendMissionEvents(buildFixtureProviderNormalizedHistoryAuditInputs({ missionId: input.missionId, turnId: session.id, normalizedEvents }));
