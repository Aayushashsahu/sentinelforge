...
import { bindFixtureProofApprovalCheckpoint } from "./liveFixtureProof";
...
export async function runLiveFixtureProofApprovalCapture(input: { missionId: string; actionId: string }) {
  ...
  const events = await readBoundedTurn({ client, sessionId: session.id, message: buildFixtureProofApprovalMessage({ owner, repo }) });
  const { pause, turnId } = validateFixtureProofApprovalCaptureSequence(events, config.toolsMcpName);
