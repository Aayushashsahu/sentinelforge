...
export type FixtureProofAction = {
  ...
  failure?: GitHubWriteFailureEvidence;
};
...
export type FixtureProofPersistencePort = {
  ...
  updateAction(actionId: string, update: Pick<FixtureProofAction, "status" | "remote" | "failure">): Promise<void>;
...
function safeRemoteSummary(action: FixtureProofAction): Record<string, unknown> {
  return { actionId: action.id, branchName: action.intent.branchName, repository: action.intent.repository, file: action.intent.filePath, remote: action.remote, failure: action.failure ?? null };
}
