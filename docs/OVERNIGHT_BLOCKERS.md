...
## 2026-08-25 — First-party Repair Engineer proposal supersedes the former external-MCP repair limitation

The selected release-incident path no longer depends on the legacy `github` MCP for file body delivery. The runtime owner registered `sentinelforge-tools` with exactly `get_repository`, `get_file`, `get_issue`, and `get_workflow_run`, and SentinelForge verified the active catalog before a separately authorized Repair Engineer session. That one session (`01m0wbj50nj21txhwzj9fk05qb` / `01m0wbj6v8f0ych1e2aczmjtb9.local`) called first-party `get_file` for `release-manifest.json` and `package.json`; ordinary text bodies reached the model.

The final response proposed the bounded `release-manifest.json` version change from `1.3.0` to `1.4.0`, but represented the patch as a safe object rather than a unified-diff string and used flexible textual values for risk and evidence limitations. The original strict parser failed closed, recording a Repair Engineer parsing failure. SentinelForge made **no second turn**: it read the completed remote session history, normalized only the allowlisted manifest-version object shape, and persisted the equivalent un-applied diff with mission state restored to `PLANNING_FIX`. The persisted record has zero approval and external-action rows. No sandbox, Qodo, branch, commit, pull request, approval continuation, GitHub write, or code execution occurred.

This resolves the live Repair Engineer proposal blocker only. Sandbox verification, a genuine approval-required event, approval continuation, Qodo availability, a write-scoped GitHub path, and separate explicit authorization remain independent gates.

## First-party MCP registration prerequisite — Resolved

The release-incident pivot provides a tested first-party `sentinelforge-tools` MCP endpoint that returns decoded GitHub fixture file bodies as ordinary text. The external TrueForge runtime has registered the endpoint under the `sentinelforge-tools` name, and the running catalog exposed exactly the four intended read-only methods without required authorization. This historical prerequisite is resolved; no TrueForge-internal change was made by SentinelForge. The exact read-only method scope and remaining approval/write gates are recorded in [FIRST_PARTY_MCP_VERTICAL_SLICE.md](./FIRST_PARTY_MCP_VERTICAL_SLICE.md).
