...
| GitHub branch, commit, or pull request | **BLOCKED** | A live proposal now exists, but no real sandbox pass, real approval event, continuation, write-scoped credential, or separate write authorization exists. No external repair action was performed. |
...
> The live read-only proposal now exists. Do not advance to verifier execution, approval continuation, Qodo, or any GitHub write: sandbox verification remains blocked and each later external stage requires separately verified prerequisites and explicit authorization.
